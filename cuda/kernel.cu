#include <cuda_runtime.h>
         ...
         struct __align__(256) Slot{
             ...
             volatile uint64_t seq;         // 8 bytes - sequence number
         ...
         unified_kernel_optimized(
             SlotOptimized* __restrict__ slots,
             HeaderOptimized* __restrict__ hdr,
             const StreamContextOptimized* __restrict__ contexts,
             const uint32_t n_messages,
             KernelMetricsOptimized* __restrict__ metrics
         ) {
             // Thread identifiers
             ...
             for (uint32_t msg_base = bid * BLOCK_SIZE;
                 ...
                 SlotOptimized* my_slot = &slots[slot_idx];
         ...
         persistent_kernel_ultra(
             SlotOptimized* __restrict__ slots,
             HeaderOptimized* __restrict__ hdr,
             StreamContextOptimized* __restrict__ work_queue,
             ...
             while (spins++ < spin_limit) {
                 ...
                 StreamContextOptimized* work = &work_queue[work_id];
                 ...
                 SlotOptimized* slot = &slots[slot_idx];
         ...
         extern "C" int launch_optimized_kernel(
             ...
             KernelMetricsOptimized* d_metrics = nullptr;
             if (enable_metrics) {
                 cudaMalloc(&d_metrics, blocks * sizeof(KernelMetricsOptimized));
             }
             ...
             if (enable_metrics) {
                 unified_kernel_optimized<256, 192, true><<<blocks, threads_per_block, shared_size, stream>>>(
                     (SlotOptimized*)slots,
                     (HeaderOptimized*)hdr,
                     (StreamContextOptimized*)contexts,
                     n_messages,
                     d_metrics
                 );
             } else {
             ...
             if (enable_metrics && d_metrics) {
                 ...
                 // Process metrics
                 KernelMetricsOptimized* h_metrics = new KernelMetricsOptimized[blocks];
                 cudaMemcpy(h_metrics, d_metrics, blocks * sizeof(KernelMetricsOptimized),
         ...
         extern "C" int launch_ultra_persistent_kernel(
             ...
             persistent_kernel_ultra<4><<<blocks, threads, 0, stream>>>(
                 (SlotOptimized*)slots,
                 (HeaderOptimized*)hdr,
                 (StreamContextOptimized*)work_queue,
