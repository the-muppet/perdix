...
impl<'a> Producer<'a> {
    ...
    pub fn try_produce(&mut self, data: &[u8]) -> Result<u64, &'static str> {
        ...
        unsafe {
            ...
            // For small buffers, leave 1 slot empty; for large buffers, leave more margin
            let margin = if n_slots <= 64 { 1 } else { 64 };
            if (write_idx - read_idx) >= (n_slots - margin) {
...
impl<'a> Consumer<'a> {
    ...
    pub fn new(arc_pinned: Arc<Pinned>) -> Self {
        ...
        // Start from the current read index in the header
        let read_seq = unsafe { (*header_ptr).consumer.read_idx.load(std::sync::atomic::Ordering::Acquire) };
    ...
    pub fn try_consume(&mut self) -> Option<Message> {
        unsafe {
            ...
            // UINT64_MAX means slot is empty/unwritten
            if slot_seq == u64::MAX {
                return None;  // Slot not written yet
            }

            // Check sequence matches what we expect
