...
impl Buffer {
    ...
    pub fn new(n_slots: usize) -> Result<Self, String> {
        ...
        // No GPU available - this is a GPU-only library!
        Err("No GPU acceleration available. Perdix requires either CUDA or WebGPU support.".to_string())
    }
    
    #[cfg(feature = "webgpu")]
    fn new_webgpu(n_slots: usize) -> Result<Self, String> {
        // WebGPU doesn't support unified memory like CUDA
        // Would require a completely different architecture with:
        // 1. Staging buffers for CPU access
        // 2. Explicit GPU->CPU copies
        // 3. Different synchronization model

        // This fundamentally breaks the zero-copy design of Perdix
        Err("WebGPU backend requires a different architecture and is not yet implemented. Use CUDA for zero-copy GPU-CPU shared memory.".to_string())
    }
