...
extern "C" int launch_unified_kernel_async(
    ...
    // return immediately for true streaming
    return 0;
}
...
__global__ void robust_test_kernel(
    ...
    for (int msg_id = tid; msg_id < n_messages; msg_id += stride) {
        ...
        // Atomically reserve a unique sequence number
        ...
        // Verify slot addresses
        if (seq == 20 || seq == 25) {
            printf("Robust: seq=%llu -> slot_idx=%llu, slot addr=%p\n", 
                   seq, slot_idx, (void*)slot);
        }
        
        // Write payload and metadata (everything except seq)
        ...
        // Publish by writing sequence number last
